...
# Credenciales de Brevo configuradas mediante variables de entorno.
EMAIL_SENDER = os.environ.get("EMAIL_SENDER", "")
BREVO_API_KEY = os.environ.get("BREVO_API_KEY", "")

def send_email_brevo(to, subject, html, text, sender_name="Comfandi Yumbo") -> bool:
    """
    Envia un correo electronico utilizando la API v3 de Brevo con requests.
    ...
    # Payload para la API de Brevo v3
    payload = {
        "sender": {
            "name": sender_name,
            "email": EMAIL_SENDER
    ...
    try:
        ...
        if 200 <= response.status_code < 300:
            print(f"[Correo] Correo enviado exitosamente a {destinatarios} via Brevo (Status: {response.status_code})")
            return True
        else:
...
def enviar_correos_nuevo_agendamiento(
    ...
    for intento in range(1, max_intentos + 1):
        try:
            ...
            print(f"[Correo] Intento {intento} fallido en envio Brevo combinado: {e}")
            if intento < max_intentos:
                time.sleep(2)
    return False
